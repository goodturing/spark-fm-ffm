...
object TestFFM extends App {
  ...
  override def main(args: Array[String]): Unit = {
    ...
    val ffm: FFMModel = FFMWithAdag.train(training, m, n,
      dim = (args(6).toBoolean, args(7).toBoolean, args(1).toInt),
      n_iters = args(2).toInt,
      eta = args(3).toDouble,
      regParam = (args(4).toDouble, args(5).toDouble),
      normalization = false, random = false, "adagrad"
    )

    val scores: RDD[(Double, Double)] = testing.map(
      x => {
